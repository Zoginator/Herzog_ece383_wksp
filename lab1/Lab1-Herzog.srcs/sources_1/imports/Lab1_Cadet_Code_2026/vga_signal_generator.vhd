...
-- Glue code to connect the horizontal and vertical counters
   --connect horizontal roll to vertical ctrl
...
begin
   if (rising_edge(clk)) then
        ...
        elsif (current_pos.col >= 0) and (current_pos.col < 638) then
            h_sync_is_low <= false;
            h_blank_is_low <= true;
        elsif (current_pos.col >= 638) and (current_pos.col < 654) then
            h_sync_is_low <= false;
            h_blank_is_low <= false;  
        elsif (current_pos.col >= 654) and (current_pos.col < 750) then
            h_sync_is_low <= true;
            h_blank_is_low <= false;
        elsif (current_pos.col >= 750) and (current_pos.col < 798) then
            h_sync_is_low <= false;
            h_blank_is_low <= false;
        elsif (current_pos.col >= 798) then
            h_sync_is_low <= false;
            h_blank_is_low <= true;    
        end if;
        ...
        if (reset_n = '0') then
            ...
            v_sync_is_low <= true;
            v_blank_is_low <= false;
        elsif (current_pos.row >= 491) and (current_pos.row <= 524) then
            v_sync_is_low <= false;
            v_blank_is_low <= false;
        elsif (current_pos.col >= 798) and (current_pos.row = 525) then
            v_sync_is_low <= false;
            v_blank_is_low <= true;    
        end if;  
